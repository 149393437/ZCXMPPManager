...
- (void)beginRecordByFileName:(NSString*)_fileName;{
    ...
    _isFinish=NO;
    [UIView showView:_recorderView animateType:AnimateTypeOfPopping finalRect:CGRectMake([UIScreen mainScreen].bounds.size.width/2-80, [UIScreen mainScreen].bounds.size.height/2-80, 160, 160) completion:^(BOOL finish){
        if (finish){
            //注册nScreenTouch事件
...
- (void)initRecordView{
        ...
        _recorderView = (ChatRecorderView*)[[[NSBundle mainBundle]loadNibNamed:@"ChatRecorderView" owner:self options:nil]lastObject];
        _recorderView.layer.cornerRadius=10;
        _recorderView.layer.masksToBounds=YES;
    //还原界面显示
    [_recorderView restoreDisplay];
}
#pragma mark - 启动定时器
- (void)startTimer{
...
- (void)updateMeters{
    ...
    if (recorder.isRecording){
        ...
        [_recorderView updateMetersByAvgPower:[recorder averagePowerForChannel:0]];

        //倒计时
        if (curCount >= maxRecordTime - 10 && curCount < maxRecordTime) {
            //剩下10秒
            _recorderView.countDownLabel.text = [NSString stringWithFormat:@"录音剩下:%d秒",(int)(maxRecordTime-curCount)];
...
- (void)touchMoved:(CGPoint)_point{
    ...
    //设置取消动画
    [_recorderView prepareToDelete:canNotSend];
}
#pragma mark - 触摸结束
- (void)touchEnded{
    if (_isFinish==NO) {
        //正在执行动画
        [self performSelector:@selector(touchEnded) withObject:nil afterDelay:1];
        return;
    }
